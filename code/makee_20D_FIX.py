...
LEARN_RATE = 0.0001  # [0.00001, 0.0001]
...
class XesEventLog:
    ...
    def dfg_to_matrix(self, attribute, dfg):
        ...
        Returns
        -------
            matrix_relation: Dataframe, 活动之间的关联矩阵; 有向, 方向为从行名到列名
            matrix_weight: Dataframe, 活动之间的权重矩阵; 有向, 方向为从行名到列名
        """
...
def train_model(model_path_fold, dfg_relations_, train_prefix_weights_, valid_prefix_weights_,
                train_prefix_encodings_fold_, valid_prefix_encodings_fold_, train_labels_fold_,
                valid_labels_fold_):
    ...
    integrated_train.append(torch.tensor(train_prefix_encodings_fold_, dtype=torch.float))
    integrated_train.append(torch.tensor(train_labels_fold_, dtype=torch.float))
    for attribute in ATTRIBUTES:
        integrated_train.append(torch.tensor(train_prefix_weights_[attribute], dtype=torch.float))

    integrated_valid.append(torch.tensor(valid_prefix_encodings_fold_, dtype=torch.float))
    integrated_valid.append(torch.tensor(valid_labels_fold_, dtype=torch.float))
    for attribute in ATTRIBUTES:
        integrated_valid.append(torch.tensor(valid_prefix_weights_[attribute], dtype=torch.float))
    ...
    net = XGCN(dfg_relations_, config)
    if USE_CUDA:
        net.cuda()
...
def run_test(net, test_prefix_weights_, test_prefix_encodings_, test_labels_):
    integrated_test = []
    ...
    test_macro_pr = metrics.precision_score(all_labels_test, all_predict_test, average='macro', zero_division=0)
    test_macro_re = metrics.recall_score(all_labels_test, all_predict_test, average='macro', zero_division=0)
    test_macro_f = metrics.f1_score(all_labels_test, all_predict_test, average='macro', zero_division=0)

    test_write_add = os.path.basename(__file__)[:-3] + "_result.csv"
    with open(test_write_add, 'a', encoding='utf8') as f:
        f.write(
            f'{test_loss},{test_acc},{test_macro_pr},{test_macro_re},{test_macro_f}\n')
    f.close()

    print(f'results ===>>> '
          f'test_loss: {test_loss} | test_acc: {test_acc} | test_macro_pr: {test_macro_pr} | '
          f'test_macro_re: {test_macro_re} | test_macro_f: {test_macro_f}\n')
    return test_acc
...
def main():
    ...
    for k in range(FOLD):
        ...
        tk_start = time.time()
        model_path_fold = DATASET + "_" + os.path.basename(__file__)[:-3] + "_" + str(k + 1) + ".pth"
        train_model(model_path_fold, dfg_relations, train_prefix_weights, valid_prefix_weights,
                    train_prefix_encodings_fold, valid_prefix_encodings_fold, train_labels_fold,
                    valid_labels_fold)
        net = torch.load(model_path_fold, map_location=DEVICE)
        net.cpu()
        print(f"-----------------------测试模型: fold {k + 1}-----------------------")
        test_prefix_weights = dict()
        for attribute in ATTRIBUTES:
        ...
        run_test(net, test_prefix_weights, test_prefix_encodings, test_labels)
        tk_end = time.time()
        print(f"-----------------------fold{k + 1}用时: {tk_end - tk_start}-----------------------")
...
if __name__ == "__main__":
    ...
    print("===============================数据处理===============================")
    # 数据处理
    data_pro(chosen_attributes=ATTRIBUTES)  # 列表chosen_attributes的第一个属性必须为活动属性
    # 模型训练
    print("===============================模型训练===============================")
    main()
    T1 = time.time()
